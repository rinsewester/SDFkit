...
class MainWindow(QWidget):
    ...
    def initUI(self):
        ...
        self.gwid = GraphWidget()
        self.gwid.setGraph(G0)
        ...
        self.scrlarea.setWidget(self.gwid)

        self.btnBack = QPushButton("Back", self)
        self.btnBack.setEnabled(False)
        self.btnBack.clicked.connect(self.btnBackNextClicked)

        self.btnNext = QPushButton("Step", self)
        self.btnNext.clicked.connect(self.btnBackNextClicked)

        self.lblStateNr = QLabel('State: 0', self)
        self.lblStateNr.setMinimumWidth(64)
        ...
        hbox.addWidget(self.lblStateNr)
        hbox.addWidget(self.btnNext)
        hbox.addStretch()
        vbox.addLayout(hbox)
        self.setLayout(vbox)

        self.setGeometry(2500, 300, 600, 600)
        self.setWindowTitle('SDF graph simulator')
        self.show()

    def btnBackNextClicked(self):

        if self.sender() == self.btnBack:
            G0.back()
        else:
            G0.step()

        self.gwid.update()
        self.lblStateNr.setText('State: ' + str(G0.iterCount()))
        self.btnBack.setEnabled(G0.iterCount() > 0)
