...
class CSDFGraph(nx.DiGraph):
    ...
    def __init__(self, name=''):
        ...
        # Create the file for the python node functions and import it
        with open('pythonnodefunctions.py', 'w') as pnffile:
            pnffile.write('# Autogenerated file containing the functions executed by the nodes.\n\nfuncs = {}')
            import pythonnodefunctions
    ...
    def updateNodeFunction(self, nodename, funcstr):
        # TODO add validation
        m = re.match(r'^def\s+(\w+)\(.*$', funcstr, re.DOTALL | re.MULTILINE)
        if m == None:
            raise SyntaxError('Cannot find name of function for node ' + nodename + ': \n' + funcstr)
        fname = m.group(1)

        # update the function 
        self.node[nodename]['funcstr'] = funcstr

        # Write al ptyhon code into a file and import it for graph simulation
        pycodestr = '# Autogenerated file containing the functions executed by the nodes.\n\n'
        for n in self.nodes():
            pycodestr += self.node[n]['funcstr'] + '\n\n'
            

        with open('pythonnodefunctions.py', 'w') as pnffile:
            pnffile.write(pycodestr)
            import pythonnodefunctions
            il.reload(pythonnodefunctions)

        # Update the reference to the function for the current node
        # self.node[nodename]['func'] = eval('pythonnodefunctions.' + fname)
        print('update of node function for:', nodename, 'with function', fname, 'function is lib:', dir(pythonnodefunctions))
