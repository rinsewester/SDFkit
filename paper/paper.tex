...
\label{sec:approach}
  ...
  Even though no standard C for-loops are used to express repettive computations, this should not be a problem for programmers since similar patters are also used in map-reduce applications.
  Figure XXXXXXX shows two examples of computational structures: the higher-order function \emph{zipWith} and \emph{foldl}.
  ...
  \missingfigure{plaatje zipw en foldl hier}

  \todo[inline]{Robert, kun je hier wat vertellen over de transformaties van computational structures naar CSDF graphs en bijbehorende transformaties, inclusief voorbeeld}
  
  In order to determine accuracte perfomance metric of an application, the simulation must be accurate.
  In SDFkit, the simulation can be used to for verify functional correctness as wel as performance.
  ...
  \todo[inline]{hardware generation using \clash \cite{Baaij10}}
  nodes becaome combinational components and adges are implemented as FIFOs.....
  The code is generated using \clash, a fundtional hardware description language.
  each node has a peace of \clash code that represents the functionality of the node.....
